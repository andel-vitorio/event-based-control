...
def closed_loop_detm(x0, config, results):
  """
  Executa uma simulação completa para o estado inicial 'x0' usando DETM.
  """
...
def closed_loop_setm(x0, config, results):
  """
  Executa uma simulação completa para o estado inicial 'x0' usando SETM.
  """
  ...
  try:
    ...
    while sim.advance_clock():
      ...
      with sim.step(default_inputs=default_inputs) as step:
        ...
        event_time += [sim.t]
        uc = controller.compute(setm.x_hat, rho_hat)
    ...
    return (plant_output, time_history, signal_control, event_time)
...
def closed_loop_setmst(x0, config, results):
  """
  Executa uma simulação completa para o estado inicial 'x0' usando SETM.
  """
...
def closed_loop_aetm(x0, config, results):
  """
  Executa uma simulação completa para o estado inicial 'x0' usando AETM.
  """
  ...
  try:
    ...
    Xi = results['etm']['Ξ']
    sigma_til = results['etm']['σ_til']
    K = results['controller']['K']

    design_params = config["design_params"]["dspetc"]
    sigma0 = design_params.get('σ0', 0.05)
    u_bar = plant.get_input_bounds()
    sampler = ds.Sampler(Ts=design_params['h'], time_source=lambda: sim.t)

    aetm_kwargs = {'Ξ': Xi, 'σ_til': sigma_til}
    aetm = DSPETC.AETM(**aetm_kwargs)
    sim.add_system(aetm)
    ...
    while sim.advance_clock():
      ...
      with sim.step(default_inputs=default_inputs) as step:
        transmit = False
        if sampler.check():
          aetm.xm = plant.states.copy()
          transmit = aetm.triggering_condition() or (sim.t == 0.0)

        if not transmit:
          continue

        rho_hat = plant.evaluate_parameters(sim.t)
        aetm.x_hat = aetm.xm.copy()
        event_time += [sim.t]
        uc = controller.compute(aetm.x_hat, rho_hat)
    ...
    sim.finalize_history()
    plant_output = sim.output_history['plant']
    sigma_hist = sim.output_history['AETM']
    time_history = sim.time_history

    return (plant_output, time_history, signal_control, sigma_hist, event_time)
