...
def open_loop(x0, config, u_constant=0.0):
  ...
  if x0_arr.shape[0] != A.shape[0]:
    raise ValueError(
        f"Dimensão de x0 ({x0_arr.shape[0]}) incompatível com a planta ({A.shape[0]}).")
...
def _kernel_closed_loop_setm(
  ...
  for k in range(n_steps):
    if k % steps_per_sample == 0:
      ...
      time_since_last = (k - last_event_k) * dt
      should_trigger = (xm.T @ Ψ @ xm - ε.T @ Ξ @ ε <
                        0) or (time_since_last >= iet_max - 1e-9) or (k == 0)
...
def closed_loop_setm(x0, config, results):
  ...
  iet_max = float(config["design_params"].get('iet_max', 1e12))
...
def _search_next_event(x_tk, Ad, Bd, K, Xi, Psi, max_lookahead):
  ...
  # Se atingiu o max_lookahead (seja por fim de horizonte ou iet_max),
  # precisamos calcular o estado neste instante final.
  x_mh = Ad @ x_mh + Bd @ u_hold
...
def _kernel_recurrence_map(
  ...
  while current_step_idx < total_duration_steps:
    ...
    lookahead = remaining_steps
    if iet_max_steps < lookahead:
      lookahead = iet_max_steps
...
def recurrence_model(x0, config, results):
  ...
  iet_max = float(config["design_params"].get('iet_max', 1e12))
  iet_max_steps = int(round(iet_max / h))
  duration = float(config['duration'])
