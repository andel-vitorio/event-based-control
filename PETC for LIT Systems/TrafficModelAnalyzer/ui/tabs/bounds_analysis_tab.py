...
class BoundsAnalysisTab(QWidget):
  """
  Analyzes and visualizes absolute and asymptotic limit bounds of event-based control systems,
  ...
  def _setup_ui(self) -> None:
    ...
    self.plot_widget = pg.PlotWidget(
        title="System Limit Bounds vs. Parameter")
    self.plot_widget.setLabel('left', 'Bound Values')
    ...
    table_label = QLabel("Comparative Bounds Metrics")
    ...
    self.metrics_table.setHorizontalHeaderLabels([
        "K-Max", "Design Approach", "L", "Absolute Min (Inf)",
        "Absolute Max (Sup)", "Avg InfLimAvg", "Avg SupLimAvg"
    ])
  ...
  def _render_graph(self) -> None:
    """
    Constructs a multi-line chart with unique markers evaluating system limit bounds.
    """
    self.plot_widget.clear()
    self.legend.clear()
    ...
    required_cols = ['Inf', 'Sup', 'InfLimInf',
                     'SupLimSup', 'InfLimAvg', 'SupLimAvg']
    if not all(col in df.columns for col in required_cols):
      return

    df[x_col] = pd.to_numeric(df[x_col], errors='coerce')
    df = df.dropna(subset=[x_col] + required_cols)
    df = df[df[x_col] > 0.0]

    df_agg = df.groupby(x_col, as_index=False)[
        required_cols].mean().sort_values(by=x_col)
    ...
    for col, hex_color, marker in plot_configs:
      y_vals = df_agg[col].values
      plot_item = pg.PlotDataItem(
          x_vals, y_vals,
          name=col,
          pen=pg.mkPen(hex_color, width=2.0),
          symbol=marker, symbolSize=8,
          symbolBrush=hex_color, symbolPen=None
      )
    ...
    self.plot_widget.plotItem.setTitle(
        f"System Limit Bounds vs. Parameter - {current_file}{title_suffix}")

  def _populate_comparative_table(self) -> None:
    """
    Caches aggregated bounds KPIs to power the comparative table.
    """
    self.raw_table_data = []
    ...
    for path in self.selected_files:
      try:
        ...
        for l_val, group in l_groups:
          min_inf = group['Inf'].min(
          ) if 'Inf' in group.columns else 0.0
          max_sup = group['Sup'].max(
          ) if 'Sup' in group.columns else 0.0
          avg_inf_lim_avg = group['InfLimAvg'].mean(
          ) if 'InfLimAvg' in group.columns else 0.0
          avg_sup_lim_avg = group['SupLimAvg'].mean(
          ) if 'SupLimAvg' in group.columns else 0.0
          ...
          self.raw_table_data.append({
              ...
              'min_inf': min_inf,
              'max_sup': max_sup,
              'avg_inf_lim_avg': avg_inf_lim_avg,
              'avg_sup_lim_avg': avg_sup_lim_avg
          })
  ...
  def _apply_table_filters(self) -> None:
    ...
    for data in self.raw_table_data:
      ...
      items = [
          ...
          QTableWidgetItem(f"{data['min_inf']:.4f}"),
          QTableWidgetItem(f"{data['max_sup']:.4f}"),
          QTableWidgetItem(f"{data['avg_inf_lim_avg']:.4f}"),
          QTableWidgetItem(f"{data['avg_sup_lim_avg']:.4f}")
      ]
