...
def setm_synthesis(
  ...
  """
  Solve the LMI-based networked control problem for SETM (Static ETM).
  ...
  # Indices: 1:x, 2:x(tk), 3:x_dot, 4:x_dot(tk), 5:error, 6:u, 7:w
  ...
  for ell in range(nu):
    for j in Bnp:
      ...
      LMISAT24 = (Ktil[j][ell:ell+1]-L2til[j][ell:ell+1]).T
      LMISAT25 = np.zeros((nx, nx), dtype=dtype)
      LMISAT31 = LMISAT13.T
      LMISAT32 = LMISAT23.T
      LMISAT33 = (u_bar[ell]**2)*np.eye(1, dtype=dtype)
      LMISAT34 = np.zeros((1, 1), dtype=dtype)
      LMISAT35 = np.zeros((1, nx), dtype=dtype)
      LMISAT41 = LMISAT14.T
      LMISAT42 = LMISAT24.T
      LMISAT43 = LMISAT34.T
      LMISAT44 = γ*δ*(u_bar[ell]**2)*np.eye(1, dtype=dtype)
      LMISAT45 = np.zeros((1, nx), dtype=dtype)
      LMISAT51 = LMISAT15.T
      LMISAT52 = LMISAT25.T
      LMISAT53 = LMISAT35.T
      LMISAT54 = LMISAT45.T
...
def setmst_synthesis(
  ...
  """
  Solve the LMI-based networked control problem for SETM* (Single-matrix Static ETM).
  ...
  def get_Λ(i, j):
    ...
    Θtil = {}
    Θtil['0'] = σ * e[2].T @ Ξtil @ e[2] - e[5].T @ Ξtil @ e[5] - Θ1 - \
        h * e[3].T @ Q1til @ e[3] + h * Θ3 - γ * e[7].T @ e[7] + \
        h * κ1.T @ Mtil @ κ1 + h * e[4].T @ Rtil @ e[4] - Θ5 + \
        nm.He(e[1].T @ Ptil @ e[4] + Fscr @ Bscr - κ2.T @ Ytil) + h * Θ4

    Θtil['h'] = σ * e[2].T @ Ξtil @ e[2] - e[5].T @ Ξtil @ e[5] - Θ1 - \
        h * Θ2 - h * e[3].T @ Q1til @ e[3] - γ * e[7].T @ e[7] - \
        h * κ1.T @ Mtil @ κ1 - Θ5 + \
        nm.He(e[1].T @ Ptil @ e[4] + Fscr @ Bscr - κ2.T @ Ytil)
...
class SETM(System):
  def __init__(self, Ξ: np.ndarray, Ψ: np.ndarray, name: str = 'SETM'):
    super().__init__(name=name, dynamics=None, output_func=None)
    self.Ξ = Ξ
    self.Ψ = Ψ
    self.xm = None
  ...
  def triggering_condition(self) -> bool:
    x = self.xm
    ε = self.x_hat - x
    val = (x.T @ self.Ψ @ x - ε.T @ self.Ξ @ ε)[0][0]
    return val < 0
...
class SETMST(System):
  def __init__(self, Ξ: np.ndarray, σ: float, name: str = 'SETMST'):
    super().__init__(name=name, dynamics=None, output_func=None)
    self.Ξ = Ξ
    self.σ = σ
    self.xm = None
  ...
  def triggering_condition(self) -> bool:
    x = self.xm
    ε = self.x_hat - x
    val = (self.σ * x.T @ self.Ξ @ x - ε.T @ self.Ξ @ ε)[0][0]
    return val < 0
